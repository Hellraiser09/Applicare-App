...
export interface IStorage {
  ...
  getAttendanceByUserIdAndDateRange(userId: number, startDate: Date, endDate: Date): Promise<Attendance[]>;
  ...
  updateDistance(id: number, distanceData: Partial<InsertDistance>): Promise<Distance | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getAttendanceByUserIdAndDateRange(userId: number, startDate: Date, endDate: Date): Promise<Attendance[]> {
    return await db
      .select()
      .from(attendance)
      .where(
  ...
  async getLocationsByUserId(userId: number): Promise<Location[]> {
    return await db
      ...
      .orderBy(locations.timestamp, "desc");
  }
  ...
  async getLocationsByUserIdAndDateRange(userId: number, startDate: Date, endDate: Date): Promise<Location[]> {
    return await db
      ...
      .orderBy(locations.timestamp, "asc");
  }
  ...
  async updateDistance(id: number, distanceData: Partial<InsertDistance>): Promise<Distance | undefined> {
    const [updatedDistance] = await db
      .update(distances)
      .set(distanceData)
      .where(eq(distances.id, id))
      .returning();
